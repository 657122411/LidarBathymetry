...
vector<int>
FindLocalMaxima(vector<float> dataVector, int minProminence, int maxProminence, int minWidth, int maxWidth) {
    ...
    for (i = 0; i < totalLocalMaxima; i++) {
        ...
        if (nextLocalMaximaIndex < totalLocalMaxima) {
            while ((nextLocalMaximaIndex < totalLocalMaxima - 1) &&
                   (localMaximaValue[nextLocalMaximaIndex] <= localMaximaValue[i]))//
            {
        ...
        /*************************** GET WIDTH ********************************/
        j = 1;
        while (((localMaximaIndex[i] - j) > 0) &&
               (dataVector[localMaximaIndex[i] - j] - minimumValue > (prominence) / 2)) { j++; }
        leftWidth = j;
        j = 1;
        while (((localMaximaIndex[i] + j) < dataVectorSize) &&
               (dataVector[localMaximaIndex[i] + j] - minimumValue > (prominence) / 2)) { j++; }
        rightWidth = j;
        int width = leftWidth + rightWidth;

        /********************* IS THE PEAK IN RANGE? ***************************/
        if ((localMaximaValue[i] - minimumValue >= minProminence) &&
            (localMaximaValue[i] - minimumValue <= maxProminence) && (width >= minWidth) && (width <= maxWidth)) {
...
void DeepWave::DeepFilter(vector<float> &srcWave, float &noise) {
    ...
    for (int i = 60; i < srcWave.size() - 60; i++) {
        if (srcWave.at(srcWave.size() - (i - 1)) > srcWave.at(srcWave.size() - i) &&
            srcWave.at(srcWave.size() - i) < srcWave.at(srcWave.size() - (i + 1))) {
            k = i;
            for (int j = i + 6/*两倍脉冲宽度*/; j < srcWave.size() - 66; j++) {
                if (srcWave.at(srcWave.size() - (j - 1)) < srcWave.at(srcWave.size() - j) &&
                    srcWave.at(srcWave.size() - j) > srcWave.at(srcWave.size() - (j + 1)))
                    l = j;
                break;
            }
            if (l > k) {
...
void DeepWave::CalcuDeepDepthByRed(vector<float> &waveParam, int &redtime, float &BorGDepth) {
    if ((waveParam.size() <= 1) || (waveParam.size() >= 5)) {
        ...
        float tbegin = (redtime < *min_element(waveParam.begin(), waveParam.end())) ? redtime : *min_element(
                waveParam.begin(), waveParam.end());
