                      ...
                      */
{
    ...
    if (!jacf) {
        ...
        RCAT("().\nIf no such function is available, use ", LEVMAR_BLEC_DIF)
        RCAT("() rather than ", LEVMAR_BLEC_DER)
        "()\n");
        return LM_ERROR;
    }
    ...
    if (!lb && !ub) {
        fprintf(stderr,
                RCAT(LCAT(LEVMAR_BLEC_DER, "(): lower and upper bounds for box constraints cannot be both NULL, use "),
                     LEVMAR_LEC_DER)
        "() in this case!\n");
        return LM_ERROR;
    }
    ...
    data.w = (LM_REAL *) malloc(
            m * sizeof(LM_REAL) + m * sizeof(int)); /* should be arranged in that order for proper doubles alignment */
    if (!data.w) {
    ...
    ret = LEVMAR_LEC_DER(LMBLEC_FUNC, LMBLEC_JACF, p, data.x, m, n + m, A, b, k, itmax, opts, info, work, covar,
                         (void *) &data);
                      ...
                      */
{
    ...
    if (!lb && !ub) {
        fprintf(stderr,
                RCAT(LCAT(LEVMAR_BLEC_DIF, "(): lower and upper bounds for box constraints cannot be both NULL, use "),
                     LEVMAR_LEC_DIF)
        "() in this case!\n");
        return LM_ERROR;
    }
