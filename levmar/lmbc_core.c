...
LNSRCH(int m, LM_REAL *x, LM_REAL f, LM_REAL *g, LM_REAL *p, LM_REAL alpha, LM_REAL *xpls,
       LM_REAL *ffpls, void (*func)(LM_REAL *p, LM_REAL *hx, int m, int n, void *adata), struct FUNC_STATE

*state,
int *mxtake,
int *iretcd, LM_REAL
stepmx,
LM_REAL steptl, LM_REAL
*sx)
{
...
BOXPROJECT(xpls, state

->lb, state->ub, m); /* project to feasible set */
...
if (fpls <= f +
slp *alpha
* lambda) { /* solution found */
*
iretcd = 0;
if (lambda == LM_CNST(1.) && sln >
...
int LEVMAR_BC_DER(
        void (*func)(LM_REAL *p, LM_REAL *hx, int m, int n,
        void *adata), /* functional relation describing measurements. A p \in R^m yields a \hat{x} \in  R^n */
void (*jacf)(LM_REAL *p, LM_REAL *j, int m, int n,
             void *adata),  /* function to evaluate the Jacobian \part x / \part p */
        LM_REAL

*p,         /* I/O: initial parameter estimates. On output has the estimated solution */
LM_REAL *x,         /* I: measurement vector. NULL implies a zero vector */
int m,              /* I: parameter vector dimension (i.e. #unknowns) */
int n,              /* I: measurement vector dimension */
        LM_REAL
*lb,        /* I: vector of lower bounds. If NULL, no lower bounds apply */
LM_REAL *ub,        /* I: vector of upper bounds. If NULL, no upper bounds apply */
        LM_REAL
*dscl,      /* I: diagonal scaling constants. NULL implies no scaling */
int itmax,          /* I: maximum number of iterations */
        LM_REAL
opts[4],    /* I: minim. options [\mu, \epsilon1, \epsilon2, \epsilon3]. Respectively the scale factor for initial \mu,
...
*/
        LM_REAL
*work,     /* working memory at least LM_BC_DER_WORKSZ() reals large, allocated if NULL */
...
if(n<m){
fprintf(stderr, LCAT(LEVMAR_BC_DER,
...
work = (LM_REAL * )
malloc(worksz
*sizeof(LM_REAL)); /* allocate a big chunk in one step */
if(!work){
fprintf(stderr, LCAT(LEVMAR_BC_DER,
"(): memory allocation request failed\n"));
return
...
if(pDp[i]!=p[i])
fprintf(stderr, RCAT(
"Warning: component %d of starting point not feasible in ",
LEVMAR_BC_DER
) "()! [%g projected to %g]\n",
i, pDp[i], p[i]);
...
for(
i = m;
i-->0; ) sp_pDp[i]=p[i]*dscl[i];
(*jacf)(sp_pDp, jac, m, n, adata); ++
njev;
...
for(
j = i + 1;
j-->0; ) /* j<=i computes lower triangular part only */
jacTjacim[j]+=jaclm[j]*
alpha; //jacTjac[i*m+j]+=jac[l*m+j]*alpha
...
*/
for(
i = j = numactive = 0, p_L2 = jacTe_inf = 0.0;
i<m;
++i){
if(ub && p[i]==ub[i]){
++
...
diag_jacTjac[i]=jacTjac[
i *m
+i]; /* save diagonal entries so that augmentation can be later canceled */
p_L2+=p[i]*p[i];
}
//p_L2=sqrt(p_L2);
...
for(
i = m;
i-->0; ) sp_pDp[i]=pDp[i]*dscl[i];
(*func)(sp_pDp, hx, m, n, adata); ++
nfev; /* evaluate function at p + Dp */
}
...
i = 0;
i<m;
++i) /* restore diagonal J^T J entries */
jacTjac[
i *m
+i]=diag_jacTjac[i];
...
{
int mxtake, iretcd;
LM_REAL stepmx, steptl = LM_CNST(1e3) * (LM_REAL)
sqrt(LM_REAL_EPSILON);

tmp = (LM_REAL)
sqrt(p_L2);
stepmx = LM_CNST(1e3) * ((tmp >= LM_CNST(1.0)) ? tmp : LM_CNST(1.0));

LNSRCH(m, p, p_eL2, jacTe, Dp, alpha, pDp,
...
gradproj; /* rather inelegant but effective way to handle LNSRCH() failures... */
}
#else
...
if(!dscl){
(*func)(pDp, hx, m, n, adata); ++
nfev; /* evaluate function at p - t*g */
}
else{
for(
i = m;
...
int itmax,          /* I: maximum number of iterations */
        LM_REAL
opts[5],    /* I: opts[0-4] = minim. options [\mu, \epsilon1, \epsilon2, \epsilon3, \delta]. Respectively the
...
*/
        LM_REAL
*work,     /* working memory at least LM_BC_DIF_WORKSZ() reals large, allocated if NULL */
LM_REAL *covar,    /* O: Covariance matrix corresponding to LS solution; mxm. Set to NULL if not needed. */
void *adata
...
data.
ffdif = !opts || opts[4] >= 0.0;

data.
func = func;
data.
hx = (LM_REAL * )
malloc(2*n*sizeof(LM_REAL)); /* allocate a big chunk in one step */
if(!data.hx){
fprintf(stderr, LCAT(LEVMAR_BC_DIF,
"(): memory allocation request failed\n"));
return
LM_ERROR;
}
...
adata = adata;
data.
delta = (opts) ? FABS(opts[4]) : (LM_REAL)
LM_DIFF_DELTA;

ret = LEVMAR_BC_DER(LMBC_DIF_FUNC, LMBC_DIF_JACF, p, x, m, n, lb, ub, dscl, itmax, opts, info, work, covar,
                    (void *) &data);
