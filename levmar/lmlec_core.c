...
static int LMLEC_ELIM(LM_REAL *A, LM_REAL *b, LM_REAL *c, LM_REAL *Y, LM_REAL *Z, int m, int n) {
    ...
    tot_sz = (a_sz + tau_sz + r_sz + worksz + Y_sz) * sizeof(LM_REAL) +
             jpvt_sz * sizeof(int); /* should be arranged in that order for proper doubles alignment */
    ...
    if (info != 0) {
        if (info < 0) {
            fprintf(stderr, RCAT(RCAT("LAPACK error: illegal value for argument %d of ", GEQP3)
            " in ", LMLEC_ELIM) "()\n", -info);
        } else if (info > 0) {
            fprintf(stderr, RCAT(RCAT("unknown LAPACK error (%d) for ", GEQP3)
            " in ", LMLEC_ELIM) "()\n", info);
        }
        free(buf);
        return LM_ERROR;
    ...
    if (rank < tn) {
        fprintf(stderr, RCAT("\nConstraints matrix in ", LMLEC_ELIM)
    ...
    if (info != 0) {
        if (info < 0) {
            fprintf(stderr, RCAT(RCAT("LAPACK error: illegal value for argument %d of ", TRTRI)
            " in ", LMLEC_ELIM) "()\n", -info);
        } else if (info > 0) {
            fprintf(stderr, RCAT(RCAT("A(%d, %d) is exactly zero for ", TRTRI)
            " (singular matrix) in ", LMLEC_ELIM) "()\n", info, info);
        }
        free(buf);
        return LM_ERROR;
    }
    ...
    if (info != 0) {
        if (info < 0) {
            fprintf(stderr, RCAT(RCAT("LAPACK error: illegal value for argument %d of ", ORGQR)
            " in ", LMLEC_ELIM) "()\n", -info);
        } else if (info > 0) {
            fprintf(stderr, RCAT(RCAT("unknown LAPACK error (%d) for ", ORGQR)
            " in ", LMLEC_ELIM) "()\n", info);
        }
        free(buf);
        return LM_ERROR;
                      ...
                      */
{
    ...
    if (!jacf) {
        ...
        RCAT("().\nIf no such function is available, use ", LEVMAR_LEC_DIF)
        RCAT("() rather than ", LEVMAR_LEC_DER)
        "()\n");
        return LM_ERROR;
    }
    ...
    if (n < mm) {
        fprintf(stderr, LCAT(LEVMAR_LEC_DER,
                             "(): cannot solve a problem with fewer measurements + equality constraints [%d + %d] than unknowns [%d]\n"),
                n, k, m);
        return LM_ERROR;
    }
    ...
    for (i = 0; i < m; ++i) {
        ...
        if (FABS(tmp - p0[i]) > LM_CNST(1E-03))
            fprintf(stderr, RCAT("Warning: component %d of starting point not feasible in ", LEVMAR_LEC_DER)
        "()! [%.10g reset to %.10g]\n",
                i, p0[i], tmp);
    }
                      ...
                      */
{
    ...
    if (n < mm) {
        fprintf(stderr, LCAT(LEVMAR_LEC_DIF,
                             "(): cannot solve a problem with fewer measurements + equality constraints [%d + %d] than unknowns [%d]\n"),
                n, k, m);
        return LM_ERROR;
    }
    ...
    for (i = 0; i < m; ++i) {
        ...
        for (j = 0, tmp = data.c[i]; j < mm; ++j)
            tmp += Zimm[j] * pp[j]; // tmp+=Z[i*mm+j]*pp[j];
        if (FABS(tmp - p0[i]) > LM_CNST(1E-03))
            fprintf(stderr, RCAT("Warning: component %d of starting point not feasible in ", LEVMAR_LEC_DIF)
        "()! [%.10g reset to %.10g]\n",
                i, p0[i], tmp);
    }
    ...
    if (covar) {
        ...
        LEVMAR_FDIF_FORW_JAC_APPROX(func, p, hx, wrk, (LM_REAL) LM_DIFF_DELTA, jac, m, n,
                                    adata); /* compute the Jacobian at p */
